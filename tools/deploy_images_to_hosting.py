...
配置ファイル名（id）:
  カード記載の正規化ID。優先順に、
    1. ocr_id       … カード画像をOCR（二重読み）で確定したID（新規カードで使用）
    2. fs_matched_id … 既存 Firestore と突合済みの確定ID（既存カードで使用）
  どちらも無いカードはスキップして警告する（IDが確定していない＝配置できない）。
  ※ fs_card_id（画像URLから機械生成した推定ID）は不正確なので使わない。
...
def card_image_id(card):
    """配置ファイル名に使う確定ID。ocr_id > fs_matched_id の順。無ければ None。"""
    return card.get("ocr_id") or card.get("fs_matched_id") or None
...
def main():
    ...
    if no_id:
        print(f"  ⚠️ ID未確定でスキップ: {len(no_id)}（OCR未実行 or 突合なし）")
    if no_url:
